            ...
            db.query(`SELECT *, concat(nc.str_LastName,", ",nc.str_FirstName," ",nc.str_MiddleName) AS str_PayorName FROM payment p JOIN breakdown b ON p.int_PaymentId=b.int_PaymentId JOIN noncitizen nc ON p.int_PayorId = nc.int_NonCitizenId JOIN natureofcollection n ON b.int_NatureOfCollectionId=n.int_NatureOfCollectionId WHERE b.int_NatureOfCollectionId=6 AND p.int_PayorType=2 AND p.int_Status=0`, (err, adoption_NC) => {
                db.query(`SELECT *, concat(po.str_PetOwnerLastName,", ",po.str_PetOwnerFirstName," ",po.str_PetOwnerMiddleName) AS str_PayorName FROM payment p JOIN breakdown b ON p.int_PaymentId=b.int_PaymentId JOIN petowner po ON p.int_PayorId = po.int_PetOwnerId JOIN natureofcollection n ON b.int_NatureOfCollectionId=n.int_NatureOfCollectionId WHERE b.int_NatureOfCollectionId IN (3,4,5) AND p.int_PayorType=3 AND p.int_Status=0`, (err, redemption_PO) => {
                    db.query(`SELECT *, concat(nc.str_LastName,", ",nc.str_FirstName," ",nc.str_MiddleName) AS str_PayorName FROM payment p JOIN breakdown b ON p.int_PaymentId=b.int_PaymentId JOIN noncitizen nc ON p.int_PayorId = nc.int_NonCitizenId JOIN natureofcollection n ON b.int_NatureOfCollectionId=n.int_NatureOfCollectionId WHERE b.int_NatureOfCollectionId IN (3,4,5) AND p.int_PayorType=4 AND p.int_Status=0`, (err, redemption_NC) => {
                        db.query(`SELECT *, concat(po.str_PetOwnerLastName,", ",po.str_PetOwnerFirstName," ",po.str_PetOwnerMiddleName) AS str_PayorName FROM payment p JOIN breakdown b ON p.int_PaymentId=b.int_PaymentId JOIN petowner po ON p.int_PayorId = po.int_PetOwnerId JOIN natureofcollection n ON b.int_NatureOfCollectionId=n.int_NatureOfCollectionId WHERE b.int_NatureOfCollectionId=8 AND p.int_PayorType=5 AND p.int_Status=0`, (err, transfer_PO) => { console.log(transfer_PO)
                            db.query(`SELECT *, concat(nc.str_LastName,", ",nc.str_FirstName," ",nc.str_MiddleName) AS str_PayorName FROM payment p JOIN breakdown b ON p.int_PaymentId=b.int_PaymentId JOIN noncitizen nc ON p.int_PayorId = nc.int_NonCitizenId JOIN natureofcollection n ON b.int_NatureOfCollectionId=n.int_NatureOfCollectionId WHERE b.int_NatureOfCollectionId=8  AND p.int_PayorType=6 AND p.int_Status=0`, (err, transfer_NC) => {
                            var adoption = {};
                            var redemption = {};
                            var unpaid = sortJsonArray(adoption_PO.concat(registration.concat(adoption_NC.concat(redemption_PO.concat(redemption_NC.concat(transfer_NC.concat(transfer_PO)))))), 'int_PaymentId', 'des');
...
router.get('/trylang', (req, res) => {
     ...
     db.query(`CALL SelectPetDetails(1)`, (err, petDetails) => {
        db.query(`CALL SelectPetOwnerDetails(${petDetails[0][0].int_PetOwnerId})`, (err, petOwnerDetails) => {
            db.query(`SELECT * FROM office`, (err, officeDetails) => {
                res.render('CVO-T-RecordCollection/views/CertificateOfTransfer_PRINT.ejs', {
                    po: petOwnerDetails,
                    pd: petDetails[0],
                    od: officeDetails,
...
router.post('/OrderOfReleaseR1/print', (req, res) => {
    ...
    db.query(`SELECT * FROM redemptiontransaction WHERE   int_RedemptionTransactionId=${req.body.redemptionTransactionId}`, (err, redemptionTransaction) => {
        console.log(redemptionTransaction)
        if (redemptionTransaction[0].int_OwnerStatus == 0) {
            db.query(`CALL SelectPetOwnerDetails(${redemptionTransaction[0].int_OwnerId});`, (err, petOwnerDetails) => {
                console.log(petOwnerDetails)
                db.query(`CALL SelectImpoundedAnimalDetails(${redemptionTransaction[0].int_AnimalId})`, (err, impoundedAnimal) => {
                    console.log(impoundedAnimal)
                    db.query(`SELECT * FROM office`, (err, officeDetails) => {
            ...
            */
            ...
            if (breakdown[0][0].int_NatureOfCollectionId == 0) {
                ...
                db.query(`SELECT * FROM redemptiontransaction WHERE int_OwnerId=${breakdown[0][0].int_PayorId} AND int_RedemptionResult=1`, (err, redemptionTransaction) => {
                    db.query(`UPDATE redemptiontransaction SET int_RedemptionResult=2 WHERE int_RedemptionTransactionId=${redemptionTransaction[0].int_RedemptionTransactionId }`, (err) => {
                        console.log(err)
              ...
              else if (breakdown[0][0].int_NatureOfCollectionId == 7) {
             ...
             else if (breakdown[0][0].int_NatureOfCollectionId == 8) {

               
                       
                            res.render('CVO-T-RecordCollection/views/TransactionSummary_T.ejs', {
